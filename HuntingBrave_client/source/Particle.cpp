...
//------------------------------------------------------------------------
	...
	//	魔法ダメージ
	void	Particle::MagicDamage(const Vector3& pos, const Vector3& back, const Vector3& side, const float& scale, const int& time)
	{
		...
		for (int j = 0; j < Par4; j++)
		{
			...
			//	ランダムで法線用ポジションをとる
			ran_pos = ideal_pos + side * (random->GetFloat(-2.0f, 2.0f));

			//	法線をセット
			Move = ran_pos - pos;
			Move.Normalize();

			Move *= (random->GetFloat(0.2, 0.3f) * scale);

			Move += Vector3(0.0f, 1.0f, 0.0f) * (random->GetFloat(-0.3f, 0.3f));


			Power = Move * (random->GetFloat(0.01f, 0.1f));
			Power.y = 0.0f;


			//	画像タイプ、出現フレーム、出現時透明度、最終フレーム、最終透明度、最高フレーム、最高透明度、出現位置、移動値、与力、	赤成分、緑成分、青成分、スケール、レンダーステート
			pt->Set(SPARK, 0, 1.0f, 25, 0.0f, 10, 1.0f, &Pos, &Move, &Power, 0.3f, 0.3f, 1.0f, scale, RS_COPY);
		}
	}

	//	剣ダメージ
	void	Particle::SwordDamage(const Vector3& pos, const float& scale)
	{
		Vector3	Pos, Move, Power;
		for (int j = 0; j<Par3; j++)
		{
			...
			Move.x = random->GetFloat(-150.0f, 150.0f) *(0.003f * scale);
			Move.y = 1.0f * scale;
			Move.z = random->GetFloat(-150.0f, 150.0f) * (0.003f * scale);

			Power.x = 0.0f;
			Power.y = random->GetFloat(-100.0f, -50.0f) * (scale * 0.001f);
			Power.z = 0.0f;

			//	画像タイプ、出現フレーム、出現時透明度、最終フレーム、最終透明度、最高フレーム、最高透明度、出現位置、移動値、与力、	赤成分、緑成分、青成分、スケール、レンダーステート
			pt->Set(SPARK, 0, 1.0f, 30, 0.0f, 20, 1.0f, &Pos, &Move, &Power, 1.0f, 0.0f, 0.0f, scale * 2.0f, RS_COPY);
		}
	}
	...
	void	Particle::Smoke(const Vector3& pos, const float& scale, const int& time)
	{
		...
		for (int j = 0; j<Par5; j++){
			...
			//	画像タイプ、出現フレーム、出現時透明度、最終フレーム、最終透明度、最高フレーム、最高透明度、出現位置、移動値、与力、	赤成分、緑成分、青成分、スケール、レンダーステート
			pt->Set(SMOG, 0, 0.0f, 60, 0.0f, 40, 1.0f, &Pos, &Move, &Power, 1.0f, 1.0f, 1.0f, scale, RS_COPY);
		}
	}
	...
	void	Particle::Semicircle(const Vector3& pos, const Vector3& front, const Vector3& right, const Vector3& up, const float radius, const float& scale)
	{
		...
		for (int j = 0; j <Par3; j++)
		{
			...
			//	画像タイプ、出現フレーム、出現時透明度、最終フレーム、最終透明度、最高フレーム、最高透明度、出現位置、移動値、与力、	赤成分、緑成分、青成分、スケール、レンダーステート
			pt->Set(SPARK, 0, 0.0f, 40, 0.0f, 20, 1.0f, &Pos, &Move, &Power, 0.2f, 0.2f, 1.0f, scale, RS_COPY);
		}
	}
