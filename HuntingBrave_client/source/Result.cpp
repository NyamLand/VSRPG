...
//----------------------------------------------------------------------------------
	...
	//	コンストラクタ
	Result::Result( void ) : back( nullptr ), view( nullptr ),
		index( -1 )
	{
	...
	//	初期化
	void	Result::Initialize( int index )
	{
		this->index = index;
	...
	//	オブジェクト初期化
	void	Result::ObjInitialize( int index )
	{
		//	テクスチャ初期化
		obj[index] = nullptr;
	...
	//	カメラ初期化
	void	Result::ViewInitialize( int index )
	{
			//	カメラ初期化
			view = nullptr;
			view = new iexView();
	...
	//	テクスチャ初期化
	void	Result::TextureInitialize( int index )
	{
	...
	//	NameUI初期化
	void	Result::NameUIInitialize( int index )
	{
		//	nameUI設定
	...
	//	PlayerTitleUI初期化
	void	Result::PlayerTitleUIInitialize( int index )
	{
		//	playerTitleUI初期化
...
//----------------------------------------------------------------------------------
	...
	//	プレイヤー称号更新
	void	Result::UpdatePlayerTitle( void )
	{
		for ( int i = 0; i < PLAYER_MAX; i++ )
		{
			if ( i != 0 )
...
//----------------------------------------------------------------------------------
	...
	//	自キャラ描画
	void	Result::MyCharacterRender( void )
	{
		//	レンダーターゲット設定
	...
	//	他キャラ描画
	void	Result::OtherPlayerRender( int index )
	{
		//	レンダーターゲット設定
	...
	//	プレイヤー称号UI描画
	void	Result::PlayerTitleUIRender( void )
	{
		for ( int i = 0; i < PLAYER_MAX; i++ )
		{
			bool active = gameParam->GetPlayerActive( order[i] );
